...
public class DigitalTwinConnectionHandler extends Thread {
    ...
    private static float[] preference_adjuster;
    private final Semaphore semaphore;
    private final int num_strat;

    public DigitalTwinConnectionHandler(Semaphore semaphore, BlockingDeque<float[]> strategy_queue, BlockingDeque<BPA[]> uncertainty_queue, BlockingDeque<ConfigurationOptions> config_queue, BlockingDeque<float[]> sensor_acc_queue, BlockingDeque<Boolean> feedback_queue, int num_strat) {
        ...
        try{
            this.preference_adjuster = new float[num_strat];
            float[] init = new float[num_strat];
            init[0] = 1;
            for(int i = 0; i < init.length; i++){
                init[i] = 0;
                preference_adjuster[i] = 10;
            }
    ...
    private void sendStrategy(DataOutputStream outputStream){
        ...
        try{
            ...
            int data = 0; // Get the latest value without removing it from the queue
